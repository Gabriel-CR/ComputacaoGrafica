...
void GuardaRoupa::desenha() {
    ...
    glPushMatrix();
        Objeto::desenha();
        glPushMatrix(); // base
            (this->selecionado) ? GUI::setColor(1, 0, 0) : GUI::setColor(0.8, 0.8, 0.8);
            glScalef(2.5, 2.5, 0.7);
            c.desenha();
        glPopMatrix();
        glPushMatrix(); // porta esquerda
            (this->selecionado) ? GUI::setColor(1, 0, 0) : GUI::setColor(0.1, 0.1, 0.1);
        ...
        glPushMatrix(); // puxador porta esquerda
            (this->selecionado) ? GUI::setColor(1, 0, 0) : GUI::setColor(1, 1, 1);
